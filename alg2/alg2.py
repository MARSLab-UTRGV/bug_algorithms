...
def is_open(yaw, atg, right, left, front):
    ...
    normalized_frontone = yaw - 10
    normalized_fronttwo = yaw + 10
...
if __name__ == "__main__":
    ...
    while robot.step(TIME_STEP) != -1:
        ...
        # checks to see if robot is aligned. if align, start moving
            ...
            if alt_turn == True:
                is_aligned = align_to_M(turn_to_angle, imu_yaw) #do 180 from current direction
            else:
                is_aligned = align_to_M(calculate_target_angle(gps_values, goal_pos), imu_yaw)
                #need to set the goal here for the object to orient itself to 
            if is_aligned: 
                prev = state
                print
                if alt_turn == True:
        ...
        # follows perimeter of obstacle.
            ...
            # following wall on the right
                ...
                if open_path and turnaround_inprogress:
                    ...
                    #print("rtg: ", rtg)
                    if (hptg > rtg):
                        #print("Direction to goal is open after turnaround")
                        leave_point.append([gps_values[0], gps_values[1]])
                        prev = state
                        state = 'align_robot_heading'
                ...
                if prev == "wall_following":
                    #print("Prev is wall follwing. test hit points.")
                    for i, hitX in enumerate(hit_point):
                        ...
                        if (abs(robot_togoal - hp_togoal) < 0.08) and (hp_to_robot < 0.08) and (safety):
                            #print("turnaround hit point is: ", hitX)
                            turnaround_point = hitX
                            #print("Hit point distance: ", calculate_euclidean_distance(hitX[0], hitX[1], gps_values[0], gps_values[1]))
                            #print("Hit point is close, turning around")
                            #hit_point = hit_point[:i+1]
                            turnaround_inprogress = True
                            if next_turn == 'left':
                                #print("Setting turn for", next_turn)
                                next_turn = 'right'
                            else:
                                next_turn = 'left'
                                #print("Setting turn for", next_turn)
            ...
            # if too far from the wall
                ...
                if next_turn == 'left':
                    ...
                    update_motor_speed(input_omega=[robot_speed/8, robot_speed])                # print("Prev Changed 2")
                prev = state
